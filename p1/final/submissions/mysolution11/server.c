...
void send_open(RPC_t* finalRPC, int sessfd){
	...
	fprintf(stderr, "open respond is: %d\n", respond);
	free(pathname);
...
void send_close(RPC_t* finalRPC, int sessfd){
	...
	int fd = close_struct -> fd - OFFSET;

	fprintf(stderr, "fd: %d\n", fd);

	int respond = close(fd);

	if (respond < 0){
		respond = -1 * errno;
	}
	fprintf(stderr, "close respond is: %d\n", respond);
...
void send_write(RPC_t* finalRPC, int sessfd){
	...
	int respond = write(fd, bufWrite, nbytes);
	if (respond < 0){
		respond = -1 * errno;
		fprintf(stderr, "server write errno: %d\n", respond);
	}
	fprintf(stderr, "server write respond is: %d\n", respond);
	char result[RETURNSIZE];
	sprintf(result, "%d", respond);
	send(sessfd, result, strlen(result), 0);
