...
struct dirtreenode* getdirtree(const char *path) {
    ...
    int len = sizeof(RPC_header_t) + sizeof(getdirtree_request_header_t) + strlen(path) + 1;
    char *buffer = malloc(len);
    ...
    getdirtree_request_header_t *getdirtree_request_header = (getdirtree_request_header_t*)rpc_header->body_ptr;
    getdirtree_request_header->path_len = strlen(path) + 1;
    memcpy(getdirtree_request_header->path_ptr, path, strlen(path)+1);
    ...
    fprintf(stderr, "send getdirtree request");
    ...
    RPC_header_t *rpc_reply = (RPC_header_t*)data;
    getdirtree_reply_header_t *getdirtree_reply = (getdirtree_reply_header_t *)rpc_reply->body_ptr;
    int path_len = getdirtree_reply->path_len;
    if (path_len < 0) {
        errno = -1 * path_len;
        return NULL;
    }
    else {
        return unmarshall_tree(getdirtree_reply);
    }
